...
{
    public class NotEnoughBallast : Report
    {
        public override void Process(Int64 distanceId, ReportTemplate template, ReportPeriod period, MetroProgressBar progressBar)
        {
                ...
                foreach (var mainProcess in mainProcesses)
                {
                    foreach (var track_id in admTracksId)
                    {
                        ...
                        foreach (var km in kms)
                        {
                            ...
                            var digression = "недостаточность балласта в шпальных ящиках";
                            var percent = "50%";
                            var fastening = "КБ";
                            var length = 0;
                            ...
                            {
                                if (Math.Min(Ballast[i].Right_data, Ballast[i].Left_data) > 70)
                                    length++;
                            }

                            var calc = (int)Math.Abs(((Ballast.First().Km + (double)Ballast.First().Meter / 10000) - (Ballast.Last().Km + (double)Ballast.Last().Meter / 10000)) * 10000);
            ...
            try
            {
                htReport.Save(Path.GetTempPath() + "/report_NotEnoughBallast.html");
            }
            catch
            {
            ...
            {
                System.Diagnostics.Process.Start(Path.GetTempPath() + "/report_NotEnoughBallast.html");
            }
        }
    }
}
