...
{
    public class DeviationsInRails : Report
    {
        public override void Process(Int64 distanceId, ReportTemplate template, ReportPeriod period, MetroProgressBar progressBar)
        {
            ...
            using (XmlWriter writer = htReport.CreateWriter())
            {
                ...
                {
                    if(string.IsNullOrEmpty(mainProcess.DistanceName)) 
                        mainProcess.DistanceName =  distance.Code; 

                    if (string.IsNullOrEmpty(mainProcess.TrackName))
                        mainProcess.TrackName = AdmStructureService.GetTrackName(mainProcess.TrackID)?.ToString();
                  
                    var digressions = RdStructureService.GetDigressions(mainProcess, new int[] { 1028, 1029, 1030, 1031, 1032, 1033, 1034, 1035 });
                    if (digressions == null || digressions.Count < 1)
                    {
                        continue;
                    }
                   ...
                   else
                    {
                        ...
                        foreach (var finddeg in digressions)
                        {
                            XElement xeElements = new XElement("elements",
                                new XAttribute("n", index),
                                new XAttribute("pchu", "ПЧУ-" + finddeg.PCHU + "/ПД-" + finddeg.PD + "/ПДБ-" + finddeg.PDB),
                                new XAttribute("station", finddeg.StationName == null ? "": finddeg.StationName),
                                new XAttribute("km", finddeg.Km),
                                new XAttribute("piket", finddeg.Meter / 100 + 1),
                                new XAttribute("meter", finddeg.Meter),
                                new XAttribute("speed", finddeg.FullSpeed == null ? "" : finddeg.FullSpeed),
                                new XAttribute("digression", finddeg.Name == null ? "" : finddeg.Name),
                                new XAttribute("size", finddeg.Value),
                                new XAttribute("addspeed", finddeg.AllowSpeed == null ? "" : finddeg.AllowSpeed),
                                new XAttribute("notice", finddeg.Primech == null ? "" : finddeg.Primech));
                            xeTracks.Add(xeElements);
            ...
            try
            {
                htReport.Save(Path.GetTempPath() + "/report_DeviationsInRails.html");
            }
            catch
            {
            ...
            {
                System.Diagnostics.Process.Start(Path.GetTempPath() + "/report_DeviationsInRails.html");
            }
        }
    }
}
